% Exemplo de dissertação do INF-UFG com texto em portugues formatado com LaTeX
\documentclass[monografia]{inf-ufg}
% Opções da classe inf-ufg (ao usar mais de uma, separe por vírgulas)
%   [tese]         -> Tese de doutorado.
%   [dissertacao]  -> Dissertação de mestrado (padrão).
%   [monografia]   -> Monografia de especialização.
%   [relatorio]    -> Relatório final de graduação.
%   [abnt]         -> Usa o estilo "abnt-alf" de citação bibliográfica.
%   [nocolorlinks] -> Os links de navegação no texto ficam na cor preta.
%                     Use esta opção para gerar o arquivo para impressão
%                     da versão final do seu texto!!!
%----------------------------------------------------- INICIO DO DOCUMENTO %
%% Lista de siglas em mais de uma página
\usepackage{array,booktabs}
\usepackage{longtable}
\usepackage{pdfpages}
\usepackage{graphicx}
%\usepackage{subcaption}
\begin{document}

%------------------------------------------ AUTOR, TÍTULO E DATA DE DEFESA %
\autor{Jean Teixeira Lima} % (José da Silva)
\autorR{Teixeira, Jean Lima} % (da Silva, José)
\autorB{Igor Rodrigues Vieira}
\autorBR{Vieira, Igor Rodrigues}
\autorC{Marcello Henrique Dias de Moura}
\autorCR{de Moura, Marcello Henrique Dias}

\titulo{Proposta de um modelo de referência para elaboração e gerenciamento de um Catálogo de Serviços de TI na UFG}
%\subtitulo{Definição de um modelo de referência para elaboração e gerenciamento de Catálogo de Serviços de TI na UFG}

\cidade{Goiânia} % Nome da cidade em foi desenvolvido o trabalho
\dia{21} %
\mes{Outubro} % Data da apresentação/defesa do trabalho
\ano{2019} % Formato numérico: \dia{01}, \mes{01} e \ano{2009}

%-------------------------------------------------------------- ORIENTADOR %
\orientador{Me. Edison Andrade Martins Morais}
\orientadorR{Andrade Martins Morais, Edison}
% Use os comandos a seguir se for Orientadora e nao Orientador.
%\orientadora{\textless Nome da Orientadora\textgreater}
%\orientadoraR{\textless Nome Reverso da Orientadora\textgreater}

\coorientador{Dr. Eliomar Araújo de Lima}
\coorientadorR{de Lima, Eliomar Araújo}
% Use os comandos a seguir se for Co-orientadora e nao Coorientador.
%\coorientadora{\textless Nome da Co-orientadora\textgreater}
%\coorientadoraR{\textless Nome Reverso da Co-orientadora\textgreater}

%-------------------------------------------------- INSTITUIÇÃO E PROGRAMA %
\universidade{Universidade Federal de Goiás} % {Universidade Federal de Goiás}
\uni{UFG}         % UFG
\unidade{Instituto de Informática} %Instituto de Informática
\departamento{Pós-Graduação Lato Sensu} %Unidades com mais de um depto.

\universidadeco{Universidade Federal de Goiás}
\unico{UFG}
\unidadeco{Instituto de Informática}

\programa{Governança e Gestão de Sistemas e Tecnologias da Informação} % Computação
\concentracao{Gestão e Governança de TI}

%-------------------------------------------------- ELEMENTOS PRÉ-TEXTUAIS %
\capa    % Gera o modelo da capa externa do trabalho

%\publica % Gera a autorização para publicação em formato eletrônico
\includepdf[scale=1, offset={0 -70}]{./TCE-folha_autorizacao.pdf} %\includepdf[pages=-, pagecommand={}, width=\textwidth, frame=false]{./TCE-folha_aprovacao.pdf}

\rosto   % Primeira folha interna do trabalho

\includepdf[scale=0.9, offset={0 -70}, angle=-0.3]{./TCE-folha_aprovacao.pdf} %\includepdf[pages=-, pagecommand={}, width=\textwidth, frame=false]{./TCE-folha_aprovacao.pdf}
%\input{./tex/pre_aprovacao}
\input{./tex/pre_direitos}
\input{./tex/pre_dedicatoria}
\input{./tex/pre_agradecimentos}
\input{./tex/pre_epigrafe}
\input{./tex/pre_resumo}
\input{./tex/pre_abstract}
%\input{./tex/pre_siglas}

\tabelas[sigfigtab]
%Opções:
%nada [] -> Gera apenas o sumário
%sig     -> Gera lista de siglas
%fig     -> Gera o sumário e a lista de figuras
%tab     -> Sumário e lista de tabelas
%alg     -> Sumário e lista de algoritmos
%cod     -> Sumário e lista de códigos de programas
%
% Pode-se usar qualquer combinação dessas opções.
% Por exemplo:
%  figtab       -> Sumário e listas de figuras e tabelas
%  figtabcod    -> Sumário e listas de figuras, tabelas e
%                  códigos de programas
%  figtabalg    -> Sumário e listas de figuras, tabelas e algoritmos
%  figtabalgcod -> Sumário e listas de figuras, tabelas, algoritmos e
%                  códigos de programas

%--------------------------------------------------------------- CAPÍTULOS %
\input{./tex/cap_I}
\input{./tex/cap_II}
\input{./tex/cap_III}
\input{./tex/cap_IV}
\input{./tex/cap_V}

%------------------------------------------------------------ BIBLIOGRAFIA %
%%\cleardoublepage
%%\nocite{*} %%% Retire esta linha para gerar a bibliografia com apenas as
           %%% referências usadas no seu texto!
%\arial
\bibliography{./monografia} %%% Nomes dos seus arquivos .bib
%\bibliography{./bib/biblio} %%% Nomes dos seus arquivos .bib
%\label{ref-bib}

%--------------------------------------------------------------- APÊNDICES %
\apendices

\includepdfset{pages={-},scale=1,pagecommand={}}
\input{./tex/pos_apendices}
%\input{./pos/apend_II}

\end{document}

%------------------------------------------------------------------------- %
%        F I M   D O  A R Q U I V O :  m o d e l o - t e s e . t e x       %
%------------------------------------------------------------------------- %
